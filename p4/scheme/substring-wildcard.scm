...
(define (substring-wildcard source pattern) 
    (define f (lambda (acc source pattern flag query)
        (if (or (equal? 0 (string-length source)) (equal? 0 (string-length pattern)))
            ...
            (if (equal? (string-ref source 0) (string-ref pattern 0))
                (f (string-append acc (substring source 0 1)) (substring source 1 (string-length source)) (substring pattern 1 (string-length pattern)) #t query)
                (if (equal? #\? (string-ref pattern 0))
                    (f (string-append acc (substring pattern 0 1)) (substring source 1 (string-length source)) (substring pattern 1 (string-length pattern)) #t query)
                    (if (equal? flag #t)
                        (f "" (substring source 1 (string-length source)) (string-append acc pattern) #f query)
                        (f acc (substring source 1 (string-length source)) pattern #f query)
                    )
                )
            )
        )  
    ))
    (f "" source pattern #f pattern)
